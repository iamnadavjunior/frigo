...
function MiniKpi({ label, value, color, bg }: { label: string; value: number; color: string; bg: string }) {
  ...
  return (
    <div className={`${bg} rounded-lg px-2 py-1.5 text-center`}>
      <div className={`text-sm font-bold ${color}`}>{animated}</div>
...
export default function HistoryPage() {
  ...
  const clearFilters = () => { setDateFrom(""); setDateTo(""); setSearch(""); setSearchInput(""); };
                        ...
                        <div key={alert.id} className="bg-white dark:bg-[#141414] rounded-2xl border border-gray-200 dark:border-white/6 px-4 py-3">
                          <div className="flex items-center justify-between mb-1">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              ...
                              <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${alert.type === "REPARATION" ? "bg-red-50 dark:bg-red-500/10 text-red-500" : "bg-blue-50 dark:bg-blue-500/10 text-blue-500"}`}>
                                {alert.type === "REPARATION" ? "Réparation" : "Entretien"}
                              </span>
                              <span className={`text-[10px] font-bold ${urg.color}`}>{urg.text}</span>
                            </div>
                            <span className="text-xs text-gray-400 shrink-0">{fmtTime(alert.createdAt)}</span>
                          </div>
                          <h3 className="text-sm font-semibold text-gray-900 dark:text-white mt-1">{alert.pos.posName}</h3>
                          ...
                          {alert.description && <p className="text-xs text-gray-400 mt-1.5 line-clamp-2">{alert.description}</p>}
                          {alert.assignedTo && (
                            <div className="mt-1.5 flex items-center gap-1 text-xs text-blue-500 dark:text-blue-400">
