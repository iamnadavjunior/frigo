...
export default function FicheIndividuelPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const notes = [puissance ? `Puissance: ${puissance}` : "", fridgeType ? `Type: ${fridgeType}` : ""].filter(Boolean).join(" | ");
  ...
  function resetForm() {
    setSubmitted(false); setSelectedFridge(null); setFridgeQuery(""); setPosName(""); setAddress("");
    setOwner(""); setPhone(""); setFridgeId(""); setBrand(""); setFridgeType(""); setPuissance(""); setWorkDone(""); setError("");
  }
    ...
    return (
      <div className="flex-1 flex items-center justify-center px-3">
        <div className="text-center">
          ...
          <p className="text-[11px] text-gray-500 mb-4">Rapport de réparation envoyé à l&apos;administration.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-2.5">
          ...
          <div ref={searchRef} className="relative">
            ...
            <div className="relative">
              ...
              <input type="text" value={fridgeQuery} onChange={(e) => handleFridgeSearch(e.target.value)}
              ...
              {searching && <div className="absolute right-3 top-1/2 -translate-y-1/2"><div className="w-3.5 h-3.5 border-2 border-gray-600 border-t-teal-500 rounded-full animate-spin" /></div>}
            </div>
          ...
          <div className="bg-[#141414] rounded-lg border border-white/6 p-2.5 space-y-2">
            <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-wider">Informations du frigo</p>
            <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 mb-0.5">Type de frigo</label>
                <input type="text" value={fridgeType} onChange={(e) => setFridgeType(e.target.value)} placeholder="Type" className={inputCls} />
              </div>
              <div>
                <label className="block text-[11px] font-semibold text-gray-500 mb-0.5">Puissance</label>
          ...
          {/* ── Work Done ── */}
          <div>
            <label className="block text-[11px] font-semibold text-gray-500 mb-0.5">Travail effectué <span className="text-red-400">*</span></label>
            <textarea value={workDone} onChange={(e) => setWorkDone(e.target.value)} required rows={3}
              placeholder="Décrivez la réparation effectuée..." className={`${inputCls} resize-none`} />
          </div>
