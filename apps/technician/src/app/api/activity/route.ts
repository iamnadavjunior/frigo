...
export async function GET(request: NextRequest) {
  ...
  if (tab === "fiches" || tab === "all") {
    const fichesWhere: Record<string, unknown> = { technicianId: userId, type: "MAINTENANCE" };
    ...
    const fiches = await prisma.intervention.findMany({
      where: fichesWhere,
      select: {
        id: true, type: true, status: true, interventionDate: true, workDone: true, createdAt: true,
        refrigerator: {
          select: {
            serialNumber: true, brand: true,
    ...
    fiches.forEach((f) => results.push({ ...f, _kind: "fiche" as const }));
  }

  if (tab === "jobs" || tab === "all") {
    const jobsWhere: Record<string, unknown> = { assignedToId: userId };
    ...
    const jobs = await prisma.serviceRequest.findMany({
      where: jobsWhere,
      select: {
        id: true, type: true, urgency: true, status: true, description: true, assignedAt: true, createdAt: true,
    ...
    jobs.forEach((j) => results.push({ ...j, _kind: "job" as const }));
  }
