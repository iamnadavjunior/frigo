    ...
    <div className="min-h-screen flex flex-col bg-gray-100 dark:bg-[#0a0a0a]">
      ...
      <header className="bg-white dark:bg-[#0d0d0d] border-b border-gray-200 dark:border-white/8 sticky top-0 z-30">
        <div className="max-w-lg mx-auto px-4 pt-3 pb-2 flex items-center justify-between">
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">CABU</h1>
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              ...
              >
                <Image src="/icons/icon-192.png" alt="CABU" width={36} height={36} className="w-full h-full object-cover" />
              </button>
              {showMenu && (
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-48 bg-white dark:bg-[#1a1a1a] rounded-xl border border-gray-100 dark:border-white/10 shadow-2xl z-50 overflow-hidden py-1">
                    <div className="px-4 py-2.5 border-b border-gray-100 dark:border-white/6">
                      ...
                      <div className="text-xs text-teal-500 dark:text-teal-400 font-medium">Technicien</div>
                    </div>
                    <button
      ...
      <nav className="fixed bottom-0 inset-x-0 z-40 flex justify-center pb-5 px-4 pointer-events-none">
        <div className="pointer-events-auto w-full max-w-sm bg-white dark:bg-[#1c1c1e] rounded-4xl shadow-[0_8px_40px_rgba(0,0,0,0.15)] dark:shadow-[0_8px_40px_rgba(0,0,0,0.5)] border border-gray-100 dark:border-white/8 flex items-center px-4 h-15.5">
          {navItems.map(({ href, label, icon }) => {
            ...
            return (
              ...
              >
                {active && (
                  <span className="absolute inset-0 rounded-full bg-gray-100 dark:bg-white/12" />
                )}
                <span className={`relative transition-colors duration-200 ${active ? "text-gray-900 dark:text-white" : "text-gray-400 dark:text-gray-500"}`}>
                  {icon(active)}
                </span>
              </Link>
            );
